...
export default function Index() {
  const {setUser, setKindergardenId } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      const data = getKindergardens();
      setBarnehager(await data);
    })();
  }, []);
  ...
  const handleLogin = async (role: 'ansatt' | 'foresatt') => {
    if (!brukernavn || !passord) {
      Alert.alert('Feil', 'Vennligst fyll inn brukernavn og passord');
      return;
    }
    ...
    try {
      ...
      // Hent brukerdata fra Firestore for å sjekke rolle
      ...
      // Verifiser at brukeren har riktig rolle
      if (userData.role !== role) {
        Alert.alert('Feil', `Du er registrert som ${userData.role}, ikke som ${role}`);
        return;
      }

      if (userData.kindergardenId !== barnehage.id) {
        Alert.alert("Feil", "Bruker tilhører ikke valgt barnehage");
        return;
      }

      setUser(userCredential.user);
      setKindergardenId(userData.kindergardenId);

      // Naviger til hovedskjermen
      router.replace('/(tabs)');
    } catch (error: any) {
      let errorMessage = 'Innlogging feilet';
  ...
  const handleLoginAnsatt = () => {
    handleLogin('ansatt');
  };

  const handleLoginForesatt = () => {
    handleLogin('foresatt');
  };
  ...
  return (
    <ThemedView style={styles.container}>
      <View style={styles.content}>
        ...
        // Logg inn som ansatt knapp
        <TouchableOpacity
          style={[styles.loginButton, loading && styles.loginButtonDisabled]}
          onPress={handleLoginAnsatt}
          disabled={loading}>
          {loading ? (
        ...
        // Logg inn som foresatt knapp
        <TouchableOpacity
          style={[styles.loginButton, loading && styles.loginButtonDisabled]}
          onPress={handleLoginForesatt}
          disabled={loading}>
          {loading ? (
            <ActivityIndicator color="#fff" />
          ) : (
